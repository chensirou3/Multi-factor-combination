"""
Run joint filter mode backtest with parameter grid

This script tests the Filter mode strategy:
- Use OFI to filter (only trade when |OFI_z| < threshold)
- Use ManipScore to generate signals

Usage:
    python scripts/run_joint_filter_grid.py
...
def main():
    parser = argparse.ArgumentParser(description="Run filter mode backtest grid")
    ...
    logger.info(f"Running filter mode backtest for {args.symbol} {args.timeframe}")
    ...
    filter_params = params_cfg['filter_mode']
    
    # Build parameter grid
    param_grid = {
        'ofi_abs_z_max': filter_params['ofi_abs_z_max'],
        'manip_z_entry': filter_params['manip_z_entry'],
        'holding_bars': filter_params['holding_bars'],
    }
    ...
    import itertools
    for ofi_max, manip_entry, hold_bars in itertools.product(
        param_grid['ofi_abs_z_max'],
        param_grid['manip_z_entry'],
        param_grid['holding_bars'],
    ):
        config = FilterSignalConfig(
            ofi_abs_z_max=ofi_max,
            manip_z_entry=manip_entry,
            direction='reversal',  # ManipScore reversal logic
            holding_bars=hold_bars,
        )
        
        logger.info(f"\nTesting: OFI_max={ofi_max}, Manip_entry={manip_entry}, Hold={hold_bars}")
    ...
    output_file = output_dir / f"filter_grid_{args.symbol}_{args.timeframe}{suffix}.csv"
    ...
    for idx, row in results_df.head(10).iterrows():
        logger.info(
            f"OFI_max={row['ofi_abs_z_max']:.1f}, Manip={row['manip_z_entry']:.1f}, "
            f"Hold={row['holding_bars']}: "
